...
export const HandicapDetails = forwardRef(({ newEnseignant }, ref) => {
    ...
    const [handicapsList, setHandicapsList] = useState(() => {
        if (newEnseignant?.handicaps?.length > 0) {
            return newEnseignant.handicaps.map(h => ({
                handicap: {
        ...
        setHandicapsList(prev => prev.map((item, i) => {
            if (i === index) {
                return {
                    ...item,
                    handicap: newValue ? {
                        id_hand: newValue.handicapId || newValue.id_hand,
                        name_h: newValue.handicapName || newValue.name_h,
    ...
    const handleDialogSubmit = (index) => {
        const newHandicap = {
            handicapName: dialogValue.name_h,
            desc_h: dialogValue.desc_h,
        };
    
        myApi.addHandicap(newHandicap).then(response => {
            if (response.status === 200) {
                const addedHandicap = response.data;
                setStoredHandicaps(prev => [...prev, {
                    handicapId: addedHandicap.id,  // Adaptez selon la réponse de l'API
                    handicapName: addedHandicap.name,
                    desc_h: addedHandicap.description
                }]);
                ...
                setHandicapsList(prev => prev.map((item, i) => 
                    i === index ? {
                        ...item,
                        handicap: {
                            id_hand: addedHandicap.id || addedHandicap.handicapId,
                            name_h: addedHandicap.name || addedHandicap.handicapName,
                            desc_h: addedHandicap.desc_h || addedHandicap.description
                        }
                    } : item
                ));
            }
